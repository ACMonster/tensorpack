...
def MyInitializer(shape, dtype=dtypes.float32, partition_info=None):
    W = tf.zeros(shape)
    for in_channel in range(shape[-2]):
        for out_channel in range(shape[-1]):
            W[shape[0] / 2][shape[1] / 2][in_channel][out_channel] = 1
    return W
...
class Model(ModelDesc):
    ...
    def _get_NN_prediction(self, image):
        ...
        with argscope(Conv2D, nl=tf.nn.relu):
            l = Conv2D('denoise-conv', image, out_channel=image.get_shape().as_list()[-1], kernel_shap=3, W_init=MyInitializer)
            l = Conv2D('conv0', l, out_channel=32, kernel_shape=5)
