...
class Model(ModelDesc):
    ...
    def _get_NN_prediction(self, image):
        noise = tf.random_normal(tf.shape(image), mean = 0.0, stddev = NOISE_LEVEL)
        image = (image + noise) / 255.0
        with argscope(Conv2D, nl=tf.nn.relu):
            l, conv0_ker = Conv2D('conv0', image, out_channel=32, kernel_shape=5, need_var=True)
        ...
        return policy, value, conv0_ker

    def _build_graph(self, inputs):
        state, action, futurereward = inputs
        policy, self.value, conv0_ker = self._get_NN_prediction(state)
        ...
        reg_loss = tf.nn.l2_loss(conv0_ker, name='reg_loss')

        pred_reward = tf.reduce_mean(self.value, name='predict_reward')
        advantage = symbf.rms(advantage, name='rms_advantage')
        summary.add_moving_summary(policy_loss, xentropy_loss, value_loss, reg_loss, pred_reward, advantage)
        entropy_beta = tf.get_variable('entropy_beta', shape=[],
                initializer=tf.constant_initializer(0.01), trainable=False)
        self.cost = tf.add_n([policy_loss, xentropy_loss * entropy_beta, value_loss, reg_loss * REG_FACTOR])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise', help='noise level', default='0')
    parser.add_argument('--reg', help='regularization factor', default='0')
    args = parser.parse_args()

    NOISE_LEVEL = float(args.noise)
    REG_FACTOR = float(args.reg)
